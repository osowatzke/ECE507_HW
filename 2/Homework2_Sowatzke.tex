...
\begin{document}
	...
	\begin{enumerate}
		...
		In the saturation region, when $V_{gs} > V_t$ and $V_{ds} > V_{dsat} = V_{gs} - V_t$, the channel pinches off near the drain. In this region, the current no longer increases with $V_{ds}$.
